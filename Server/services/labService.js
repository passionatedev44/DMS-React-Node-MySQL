...
class LabService {
    ...
    async fetchMyLabs(userId) {
        const query = "SELECT * FROM labs WHERE lab_added_by = ? ORDER BY id DESC";
        const [results] = await db.promise().query(query, [userId]);
        return results;
    }

    async fetchAllLabs() {
        const query = "SELECT * FROM labs ORDER BY id DESC";
        const [results] = await db.promise().query(query);
        return results;
    }

    async fetchLabsForSchool(schoolId) {
        const query = `SELECT id, lab_name FROM labs WHERE school_id = ?`;
