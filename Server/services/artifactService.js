...
class ArtifactsService {
    async fetchDocTypes() {
        const [results] = await db
            .promise()
            .query("SELECT id, doctype_nm FROM document_type");
        return results;
    }
    ...
    async updateDocType(id, docTypeName) {
        await db
            .promise()
            .query("UPDATE document_type SET doctype_nm = ? WHERE id = ?", [
                docTypeName,
                id,
            ]);
    }
    ...
    async addNewDocType(newDocTypeName) {
        const [result] = await db
            .promise()
            .query("INSERT INTO document_type (doctype_nm) VALUES (?)", [
                newDocTypeName,
            ]);
    ...
    async deleteDocType(id) {
        await db
            .promise()
            .query("DELETE FROM document_type WHERE id = ?", [id]);
    }
    ...
    async fetchMyArtifacts(email) {
        const [results] = await db
            .promise()
            .query(
                "SELECT * FROM documents WHERE owner_author_id = ? ORDER BY id DESC",
                [email]
            );
        return results;
    }
    ...
    async deleteArtifact(id) {
        const [results] = await db
            .promise()
            .query(
                "SELECT doc_nm, doc_format, file_size FROM documents WHERE id = ?",
                [id]
            );
        const {
            doc_nm: docName,
