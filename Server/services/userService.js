...
class UserService {
    ...
    async getControlAccessInfo() {
        const query =
            "SELECT * FROM control_access WHERE side_bar_option = 'demo'";
        const [results] = await db.promise().query(query);
        console.log(results);
        return results;
    }
    ...
    async fetchControlAccessUsers() {
        const query = "SELECT * FROM control_access";
        const [results] = await db.promise().query(query);
        return results;
    }
    ...
    async updateUserControlAccess(newHasAccess) {
        const query =
            "UPDATE control_access SET has_access = ? WHERE side_bar_option = 'demo'";
    ...
    async fetchUserActivity(userId, period) {
        ...
        const [results] = await db
            .promise()
            .query(query, [userId, parseInt(period)]);
        return results;
    }
}
