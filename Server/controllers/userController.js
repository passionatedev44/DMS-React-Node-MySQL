...
exports.changeUserRole = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} changed the role of user with [ID: ${userId}]`
        );
        ...
        return res
            .status(500)
            .json({ status: "error", message: "Failed to change role" });
    }
};
...
exports.deleteUser = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} deleted the account of user with [ID: ${userId}]`
        );
        return res
            .status(200)
...
exports.getControlAccessInfo = async (req, res) => {
    ...
    try {
        ...
        if (results.length && results[0].has_access.includes(userId)) {
...
exports.updateUserControlAccess = async (req, res) => {
    ...
    try {
        const results = await userService.getControlAccessInfo();
        let hasAccess = results[0].has_access
            ? results[0].has_access.split(",")
            : [];
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} updated access control of user with [ID: ${userId}]`
        );
...
exports.fetchUserActivity = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} searched for activities [by user: ${userId}] in last ${period} days`
        );
