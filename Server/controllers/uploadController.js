...
exports.uploadDocument = async (req, res) => {
    ...
    upload.single("file")(req, res, async (err) => {
        ...
        console.log("upload Document => ", req.file);
        ...
        try {
            ...
            await documentsService.updateSystemSettings(
                "total_used_space",
                fileSize
            );
            await authService.logActivity(
                req.id,
                `User: ${req.email} uploaded a new document [${docName}]`
            );
...
exports.addUrl = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} uploaded new online document [${infoHead}]`
        );
...
exports.updateDocumentMetadata = async (req, res) => {
    ...
    try {
        ...
        await authService.logActivity(
            req.id,
            `User: ${req.email} updated a document with [ID: ${id}]`
        );
        ...
        return res
            .status(500)
            .json({ status: "error", message: "Failed to update metadata" });
    }
};
