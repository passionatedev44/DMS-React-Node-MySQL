...
export const handleChangeUserRole = async (
    ...
    if (userId === currentUserId) {
        // If logged in user tries to change its role exit the function
...
export const handleDeleteUser = async (
    ...
    if (userId === currentUserId) {
        // If logged in user tries to delete its account exit the function
...
export const getControlAcessInfo = async (setControlAccess) => {
    try {
        const response = await Axios.get(
            `${API_URL}/users/getControlAccessInfo`,
            {
                headers: {
                    Authorization: localStorage.getItem("token"),
                },
...
export const getControlAccessUsers = async (setControlAccessUsers) => {
    try {
        const response = await Axios.get(
            `${API_URL}/users/fetchControlAccessUsers`,
            {
                headers: {
                    Authorization: localStorage.getItem("token"),
                },
...
export const handleControlAccessUpdate = async (
    userId,
    setControlAccessUsers
) => {
    try {
        const response = await Axios.put(
            `${API_URL}/users/updateUserControlAccess/${userId}`,
            {},
            {
                headers: {
                    Authorization: localStorage.getItem("token"),
        ...
        if (response.data.status === "success") {
            setControlAccessUsers((prevUsers) =>
                prevUsers.map((user) =>
                    user.id === userId
                        ? { ...user, has_access: user.has_access === 1 ? 0 : 1 }
                        : user
                )
            );
            toast.success(response.data.message, {
...
export const handleUserActivitySubmit = async (
    userId,
    period,
    setUserActivity,
    setActivitySection
    ...
    try {
        const response = await Axios.post(
            `${API_URL}/users/fetchUserActivity`,
            { userId, period },
            {
                headers: {
                    Authorization: localStorage.getItem("token"),
